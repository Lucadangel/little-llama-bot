...
6. When a customer asks if you have a product (e.g. "do you have X?", "have you got X?", "X?"), ALWAYS output the show_products JSON action with X as the query — do NOT say you don't know.
...
export async function POST(req: NextRequest) {
  ...
  if (!isLikelyFaqQuestion) {
    // Short queries (1–4 words, no FAQ or escalation keywords) are treated as product searches
    const wordCount = message.trim().split(/\s+/).filter((w) => w.replace(/[^\w]/g, "").length > 0).length;
    const isShortQuery = wordCount >= 1 && wordCount <= 4;
    if (hasProductIntent || isShortQuery) {
      ...
      if (!results || results.length === 0) {
        const simplifiedQuery = message
          .toLowerCase()
          .replace(/\b(do you have|have you got|got any|have any)\b/g, " ")
          .replace(/\bany\s+/g, " ")
          .replace(/[?!]/g, "")
          .replace(
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      if (productAction) {
        ...
        const queryToSearch = query.trim() || message;
        const results = await searchProducts(queryToSearch, 5);
        if (results && results.length > 0) {
