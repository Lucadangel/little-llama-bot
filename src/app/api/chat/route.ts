...
5. For ANYTHING you are not sure about — do NOT guess or invent information. Say: "I'm not sure about that — but I'm happy to help! Try describing what you're looking for and I'll search our range. You can also type 'contact' to reach our team."
6. When a customer asks "do you have X?", "X?" or any shopping question, ALWAYS output the show_products JSON action with X as the query.
...
export async function POST(req: NextRequest) {
  ...
  // Short-query fast-path — 1–4 word messages with no FAQ/escalation keywords are product searches
  const strippedMessage = msgLower.replace(/[^a-z0-9\s]/g, "").trim();
  const wordCount = strippedMessage.split(/\s+/).filter(Boolean).length;
  const isShortQuery =
    wordCount >= 1 &&
    wordCount <= 4 &&
    !isLikelyFaqQuestion &&
    !escalationKeywords.some((kw) => msgLower.includes(kw));

  if (!isLikelyFaqQuestion && (hasProductIntent || isShortQuery)) {
    let results = await searchProducts(message, 5);

    // If no results, try a simplified query (strip question framing, keep product type)
    if (!results || results.length === 0) {
      const simplifiedQuery = message
        .toLowerCase()
        .replace(/\b(do you have|have you got)\b/g, " ")
        .replace(
          /\b(small|big|little|tiny|large|toddler|infant|newborn|interested in|i am|i want|i need|i'd like|i would like|show me|looking for|find me)\b/g,
          " "
        )
        .replace(/\?/g, "")
        .replace(/\s+/g, " ")
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      if (productAction) {
        ...
        // Bug 5: if query is empty, fall back to the original user message
        const query = productAction.query.trim() || message;
