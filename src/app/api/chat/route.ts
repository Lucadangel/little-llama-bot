...
5. For ANYTHING you are not sure about — do NOT guess or invent information. Say: "I'm not sure about that — but I'm happy to help! Try describing what you're looking for (e.g. 'blue alpaca cardigan for 6 months') and I'll search our range. You can also type 'contact' to reach our team."
6. When a customer asks "do you have X?", "X?" or similar shopping questions, ALWAYS output the show_products JSON action with X as the query — never say you don't know.
...
export async function POST(req: NextRequest) {
  ...
  // Gate 1b — Short-query fast-path: 1-4 words, no FAQ keywords → treat as product search
  const messageWords = message.replace(/[^\w\s]/g, "").trim().split(/\s+/).filter(Boolean);
  if (!isLikelyFaqQuestion && messageWords.length >= 1 && messageWords.length <= 4) {
    let results = await searchProducts(message.replace(/[?!.]+$/, "").trim(), 5);
    if (results !== null && results.length > 0) {
      return NextResponse.json({
        reply: "Here are some products that might interest you:",
        ui: { kind: "product_carousel", products: results },
      });
    }
  }
  ...
  if (!isLikelyFaqQuestion && hasProductIntent) {
    ...
    if (!results || results.length === 0) {
      const simplifiedQuery = message
        .toLowerCase()
        .replace(/\b(do\s+you\s+have|have\s+you\s+got|can\s+i\s+get|small|big|little|tiny|large|toddler|infant|newborn|interested in|i am|i want|i need|i'd like|i would like|show me|looking for|find me)\b/g, " ")
        .replace(/[?!.]+/g, " ")
        .replace(/\s+/g, " ")
        .trim();
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      if (productAction) {
        ...
        // If the LLM emitted an empty query, fall back to the original user message
        const query = rawQuery.trim() || message;
